...
exports.postCreateNewUser = async (req, res) => {
    if (!req.body.username || !req.body.email) {
        ...
        if (currentUser) {
            ...
            User.create({
                email: req.body.email,
                password: req.body.password ? req.body.password : process.env.STAFF_DEFAULT_PASSWORD,
                fullname: req.body.fullname,
                username: req.body.username,
                dob: req.body.dob,
            }).then((new_user) => {
                res.status(200).send(response.responseSuccess(new_user, true, "New user was created successfully.", "New user created successfully.", 200));
            }).catch((error) => {
                console.log(error.message);
                res.status(400).send(response.responseError(error.message, false, "Sorry! create user was not successful!", "Sorry! create user was not successful!", 400));
            });
        }
    }
};
