...
exports.createTransactionPin = async (req, res) => {
    try {
        ...
        if (!req.body.user_id || !req.body.pin) {
            ...
            return res.status(400).send(response.responseError([], false, 'Unable to create transaction pin!', message, 400));
        }
        ...
        if (!user) {
         ...
         if (!/^\d{4}$/.test(pin)) {
             ...
             return res.status(400).send(response.responseError([], false, 'Invalid PIN format!', message, 400));
         }
        ...
        return res.status(200).send(response.responseSuccess(newTransactionPin, true, 'Transaction PIN was created successfully.', 'Transaction PIN was created successfully.', 200));
    } catch (error) {
